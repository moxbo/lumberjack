...
async function testHttpPollerMemoryLeak(): Promise<void> {
  ...
  console.log(
    "Test 1: Verify seen Set is trimmed after exceeding MAX_SEEN_ENTRIES...",
  );
  ...
  const dedupeMethod = (networkService as any).dedupeNewEntries.bind(
    networkService,
  );
  ...
  console.log(
    `✓ Deduplication works correctly (${fresh.length} unique from ${testEntries.length} total)`,
  );
  ...
  if (seenLarge.size > 10000) {
    throw new Error(
      `Seen Set should be trimmed to ~5000 entries, but has ${seenLarge.size}`,
    );
  }

  console.log(
    `✓ Seen Set trimmed correctly (size: ${seenLarge.size}, max: 10000)`,
  );
