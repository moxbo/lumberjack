...
export default function DCFilterPanel() {
  ...
  useEffect(() => {
    function onDocClick(e){ if(!ctx.open) return; const el=ctxRef.current; if(el && el.contains(e.target)) return; setCtx({open:false,x:0,y:0}) }
  ...
  useEffect(() => {
    ...
    const off2 = LoggingStore.addLoggingStoreListener({
      ...
      loggingStoreReset: () => { setKeys([]); setSelectedKey('') }
    })
  ...
  useEffect(() => {
    const off = DiagnosticContextFilter.onChange(() => { setRows(DiagnosticContextFilter.getDcEntries()); setEnabled(DiagnosticContextFilter.isEnabled()) })
  ...
  function onClear() { DiagnosticContextFilter.reset(); setSel([]) }
  ...
  function openCtx(ev, id) { ev.preventDefault(); ev.stopPropagation(); if (!sel.includes(id)) setSel([id]); setCtx({open:true, x:ev.clientX, y:ev.clientY}) }

  function activateSelected(active) {
    const cur = DiagnosticContextFilter.getDcEntries(); const byId = new Map(cur.map(e => [dcEntryId(e), e]))
    for (const id of sel) { const e = byId.get(id); if (!e) continue; if (active) DiagnosticContextFilter.activateMdcEntry(e.key, e.val); else DiagnosticContextFilter.deactivateMdcEntry(e.key, e.val) }
  ...
  function onValueKeyDown(e){ if (e.key === 'F2') { e.preventDefault(); const k = String(selectedKey || '').trim(); if(!k) return; const vals = MDCListener.getSortedValues(k); setValues(vals); setShowValues(true) } }
  function chooseValue(v){ setVal(v); setShowValues(false); setTimeout(() => onAdd(), 0) }
    ...
    <div class="dc-panel" style="border-top:1px solid #ddd; padding:8px 12px;">
      <div style="display:flex; gap:12px; align-items:end; flex-wrap:wrap; justify-content:space-between;">
        <div style="display:flex; gap:12px; align-items:end; flex-wrap:wrap;">
          <div style="display:flex; flex-direction:column;">
            ...
            <input list="dc-keys" value={selectedKey} onInput={e => setSelectedKey(e.currentTarget.value)} placeholder="Key wählen oder tippen…" />
            <datalist id="dc-keys">
          ...
          <div style="display:flex; flex-direction:column; min-width:220px;">
            ...
            <input ref={valueInputRef} value={val} onInput={e => setVal(e.currentTarget.value)} onKeyDown={onValueKeyDown} title="Mögliche Werte mit F2 (Kein Eintrag findet alle MDC-Einträge mit dem Schlüssel)" placeholder="Wert oder leer für alle…" />
          </div>
        ...
        <label style="display:inline-flex; gap:6px; align-items:center; white-space:nowrap;">
          <input type="checkbox" checked={enabled} onChange={e => DiagnosticContextFilter.setEnabled(e.currentTarget.checked)} />
      ...
      <div style="margin-top:8px;">
        ...
        <div class="dc-table" style="border:1px solid #ddd; max-height:200px; overflow:auto;">
          <div class="dc-header" style="display:grid; grid-template-columns: 1fr 1fr 70px; font-weight:600; background:#f8f8f8; border-bottom:1px solid #ddd; padding:4px 8px;">
            <div>Key</div><div>Value</div><div>Active</div>
          </div>
          {rows.map(e => {
            const id = dcEntryId(e)
            const isSel = sel.includes(id)
            return (
              <div key={id} class={`dc-row${isSel?' sel':''}`} style="display:grid; grid-template-columns: 1fr 1fr 70px; padding:4px 8px; border-bottom:1px solid #eee; cursor:default;" onClick={ev => toggleRow(id, ev.shiftKey, ev.ctrlKey || ev.metaKey)} onContextMenu={ev => openCtx(ev, id)}>
                <div>{e.key}</div>
                <div>{e.val}</div>
      ...
      {ctx.open && (
        <div ref={ctxRef} class="context-menu" style={{ position:'fixed', left: ctx.x + 'px', top: ctx.y + 'px' }}>
          <div class="item" onClick={() => activateSelected(true)}>Aktiviere</div>
          <div class="item" onClick={() => activateSelected(false)}>Deaktiviere</div>
          <div class="item" onClick={() => { onRemoveSelected(); setCtx({open:false,x:0,y:0}) }}>Remove</div>
        </div>
      )}
        ...
        <div class="modal-backdrop" onClick={() => setShowValues(false)}>
          <div class="modal" onClick={e => e.stopPropagation()}>
            ...
            <div style="max-height:260px; overflow:auto; border:1px solid #eee;">
              {values.length===0 && (<div style="padding:8px; color:#777;">Keine bekannten Werte</div>)}
              {values.map(v => <div class="item" style="padding:6px 8px; border-bottom:1px solid #f0f0f0; cursor:pointer;" onClick={() => chooseValue(v)}>{v}</div>)}
            </div>
            <div class="modal-actions"><button onClick={() => setShowValues(false)}>Abbrechen</button></div>
          </div>
        </div>
      )}
    </div>
