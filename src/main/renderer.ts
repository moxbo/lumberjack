(() => {
  ...
  function markStyle(color) {
    return color ? `background: ${color};` : '';
  }

  function ensureIds(entries) {
    for (const e of entries) {
      if (!('_id' in e)) e._id = state.nextId++;
    }
  }
  ...
  function calcRange() {
    ...
    if (start < 0) start = 0;
    let end = Math.ceil((scrollTop + viewH) / rowH) + overscan;
  ...
  function renderVirtualSlice() {
    ...
    for (let vis = start; vis < end; vis++) {
      ...
      ts.className = 'col ts';
      ts.textContent = fmt(e.timestamp);
      const lvl = document.createElement('span');
      lvl.className = `col lvl ${levelClass(e.level)}`;
      lvl.textContent = fmt(e.level || '');
      const logger = document.createElement('span');
      logger.className = 'col logger';
      logger.textContent = fmt(e.logger);
  ...
  function renderDetails() {
    ...
    let raw = e.raw;
    try {
      raw = JSON.stringify(e.raw, null, 2);
    } catch (_) {}
  ...
  function allMarkedIdx() {
    const arr = [];
    for (let i = 0; i < state.entries.length; i++) if (state.entries[i]._mark) arr.push(i);
    return arr;
  }
  function gotoMark(dir) {
    const marks = allMarkedIdx();
    ...
    let target;
    if (current === -1) target = markedInOrder[0];
    else {
      const pos = markedInOrder.findIndex((i) => i === current);
      target =
        pos === -1
          ? markedInOrder[0]
          : markedInOrder[(pos + (dir > 0 ? 1 : markedInOrder.length - 1)) % markedInOrder.length];
    }
    state.selectedIdx.clear();
    state.selectedIdx.add(target);
  ...
  $('#btnOpen').addEventListener('click', async () => {
    ...
    if (res && res.ok) appendEntries(res.entries);
    else if (res && !res.ok) alert('Fehler beim Laden: ' + res.error);
  });
  ...
  $('#filterLogger').addEventListener('input', (e) => {
    state.filter.logger = e.target.value || '';
    applyFilters();
  });
  $('#filterTrace').addEventListener('input', (e) => {
    state.filter.trace = e.target.value || '';
    applyFilters();
  });
  $('#btnClearFilters').addEventListener('click', () => {
    state.search = '';
    state.filter = { level: '', logger: '', trace: '' };
    $('#searchText').value = '';
    $('#filterLevel').value = '';
    $('#filterLogger').value = '';
    $('#filterTrace').value = '';
  ...
  $('#btnMarkSelected').addEventListener('click', () => {
    ...
    if (!state.selectedIdx.size) return;
    for (const idx of state.selectedIdx) state.entries[idx]._mark = color;
    renderVirtualSlice();
  });
  $('#btnClearMarks').addEventListener('click', () => {
    for (const e of state.entries) delete e._mark;
    renderVirtualSlice();
  });
  ...
  $('#btnFilterByTrace').addEventListener('click', () => {
    ...
    if (state.selectedIdx.size > 0) {
      for (const idx of state.selectedIdx) {
        const t = state.entries[idx].traceId;
        if (t) traces.add(String(t));
      }
    }
    const val = Array.from(traces).filter(Boolean).join('|');
    $('#filterTrace').value = val;
    state.filter.trace = val;
    applyFilters();
  });
  $('#btnFilterByLogger').addEventListener('click', () => {
    const idx = [...state.selectedIdx][0];
    if (idx == null) return;
    const v = String(state.entries[idx].logger || '');
    $('#filterLogger').value = v;
    state.filter.logger = v;
    applyFilters();
  });
  $('#btnFilterByLevel').addEventListener('click', () => {
    const idx = [...state.selectedIdx][0];
    if (idx == null) return;
    const v = String(state.entries[idx].level || '');
    $('#filterLevel').value = v;
    state.filter.level = v;
    applyFilters();
  });
  $('#btnCopyMessage').addEventListener('click', async () => {
    const idx = [...state.selectedIdx][0];
    if (idx == null) return;
    const txt = String(state.entries[idx].message || '');
    try {
  ...
  // TCP controls
  $('#btnTcpStart').addEventListener('click', () => {
    const port = Number($('#tcpPort').value || 0);
    if (!port) return;
    window.api.tcpStart(port);
  });
  $('#btnTcpStop').addEventListener('click', () => window.api.tcpStop());
  window.api.onTcpStatus((s) => {
    $('#tcpStatus').textContent = s.message;
  });

  // HTTP controls
  $('#btnHttpLoad').addEventListener('click', async () => {
    const url = $('#httpUrl').value.trim();
    if (!url) return;
    const res = await window.api.httpLoadOnce(url);
    if (res.ok) appendEntries(res.entries);
    else $('#httpStatus').textContent = 'Fehler: ' + res.error;
  });
  $('#btnHttpStartPoll').addEventListener('click', async () => {
    const url = $('#httpUrl').value.trim();
    if (!url) return;
    const ms = Number($('#httpInterval').value || 5000);
    const res = await window.api.httpStartPoll({ url, intervalMs: ms });
    if (res.ok) {
      state.httpPollId = res.id;
      $('#httpStatus').textContent = `Polling #${res.id}`;
    } else $('#httpStatus').textContent = 'Fehler: ' + res.error;
  });
  $('#btnHttpStopPoll').addEventListener('click', async () => {
    if (state.httpPollId == null) return;
    const res = await window.api.httpStopPoll(state.httpPollId);
    if (res.ok) {
      $('#httpStatus').textContent = 'Poll gestoppt';
      state.httpPollId = null;
    }
  });

  // Append events from main (TCP/HTTP)
  window.api.onAppend((entries) => {
