...
function toEntry(obj = {}, fallbackMessage = '', source = '') {
  function normalizeStack(o) {
    ...
    try {
      if (o['exception.stacktrace'] != null) cand.push(o['exception.stacktrace']);
  ...
  if (trimmed.startsWith('[')) {
    try {
      ...
      if (Array.isArray(arr))
        return arr.map((o) => toEntry(o && typeof o === 'object' ? o : {}, '', filename));
    } catch (e) {
...
function buildElasticSearchBody(opts) {
  ...
  if (rawEnv) {
    ...
    if (mode === 'case-sensitive') {
      must.push({
        bool: {
          should: [{ term: { environment: rawEnv } }, { term: { 'environment.keyword': rawEnv } }],
...
function parseHitsResponse(data, size) {
  ...
  const totalVal = (() => {
    ...
    if (typeof t === 'object' && typeof t.value === 'number') return Number(t.value) || 0;
    return null;
  })();
...
async function closeScroll(
  ...
  const ok =
    (await execPost({ scroll_id: scrollId })) ||
    (await execPost({ scroll_id: [scrollId] })) ||
    (await tryDeletePath());
  if (!ok) {
