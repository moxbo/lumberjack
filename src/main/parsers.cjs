...
function tryParseJsonLoose(line) {
  try {
    ...
    const obj = {};
    const re = /"([^"\\]+)"\s*:\s*("(?:[^"\\]|\\.)*"|true|false|null|-?\d+(?:\.\d+)?)/g;
    let m;
    let found = 0;
    while ((m = re.exec(slice)) != null) {
      const key = m[1] != null ? String(m[1]) : "";
      const rawValStr = m[2] != null ? String(m[2]) : "";
      if (!key) continue;
      let val = null;
      try {
