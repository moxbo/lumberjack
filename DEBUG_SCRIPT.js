...
checks.forEach(({selector, expected, name}) => {
  ...
  const icon = ok ? '✅' : '❌';
  console.log(`  ${icon} ${name} (${selector}): ${actual} ${ok ? '' : `(expected: ${expected})`}`);
...
if (typeof getEventListeners === 'function') {
  ...
  if (listeners) {
    ...
    console.log(`  ${hasKeydown ? '✅' : '⚠️'} keydown listeners: ${listeners.keydown?.length || 0}`);
    console.log(`  ${hasMousedown ? '✅' : '⚠️'} mousedown listeners: ${listeners.mousedown?.length || 0}`);
...
if (rows.length > 0) {
  ...
  console.log(`  ${hasPointerEvents ? '✅' : '❌'} First row has pointer-events: auto`);
...
// ============= Summary =============
console.log("\n📊 Summary:");
console.log(`  Pointer Events: ${pointerEventsOk ? '✅ OK' : '❌ ISSUES FOUND'}`);
console.log(`  List Focus: ${isFocused ? '✅ OK' : '⚠️ NOT FOCUSED - click list and try again'}`);
console.log(`  Rows Rendered: ${rows.length > 0 ? '✅ YES' : '❌ NO'}`);
console.log(`  Events: ${typeof getEventListeners === 'function' ? '✅ Checkable' : '⚠️ Use Chrome/Edge'}`);
