...
// settings
let settings = { windowBounds: { width: 1200, height: 800 }, tcpPort: 5000, detailHeight: 300, colTs: 220, colLvl: 90, colLogger: 280, httpUrl: '', httpInterval: 5000, histLogger: [],  histTrace: [] };
const settingsPath = () => path.join(app.getPath('userData'), 'settings.json');
function loadSettings() {
  try {
    const p = settingsPath(); if (fs.existsSync(p)) { const raw = JSON.parse(fs.readFileSync(p, 'utf8')); settings = { ...settings, ...raw }; }
  } catch (_) {}
}
function saveSettings() {
  try {
    const p = settingsPath(); fs.mkdirSync(path.dirname(p), { recursive: true }); fs.writeFileSync(p, JSON.stringify(settings, null, 2), 'utf8');
  } catch (_) {}
}
...
function sendMenuCmd(cmd) {
  ...
  if (wc.isLoading()) { pendingMenuCmds.push(cmd); return; }
...
function makeIcon(color) {
  ...
  try { return nativeImage.createFromBuffer(canvas.toBuffer('image/png')); } catch { return null; }
}
const iconPlay = (() => { try { return makeIcon('#22c55e') } catch { return null } })();
const iconStop = (() => { try { return makeIcon('#ef4444') } catch { return null } })();
...
function buildMenu() {
  ...
  const template = [
    ...
    }] : []),
    {
      label: 'Datei',
      submenu: [
        { label: '\u00d6ffnen\u2026', accelerator: 'CmdOrCtrl+O', click: () => sendMenuCmd({ type: 'open-files' }) },
        { label: 'Einstellungen\u2026', accelerator: 'CmdOrCtrl+,', click: () => sendMenuCmd({ type: 'open-settings' }) },
        { type: 'separator' },
    ...
    },
    {
      ...
      submenu: [
        ...
        { type: 'separator' },
        { label: 'TCP Port konfigurieren\u2026', click: () => sendMenuCmd({ type: 'tcp-configure' }) },
        { type: 'separator' },
        {
          id: 'tcp-toggle',
...
function createWindow() {
    ...
    mainWindow.webContents.on('did-finish-load', () => {
      if (pendingMenuCmds.length) {
        for (const cmd of pendingMenuCmds) { try { mainWindow.webContents.send('menu:cmd', cmd) } catch {} }
        pendingMenuCmds = []
      }
    });
    ...
    mainWindow.on('close', () => {
      try { settings.windowBounds = mainWindow.getBounds(); } catch {}
      saveSettings();
    });
    ...
    if (devUrl) {
        ...
        if (fs.existsSync(distIndex)) mainWindow.loadFile(distIndex); else mainWindow.loadFile('index.html');
    }
}
...
ipcMain.handle('settings:set', (_event, patch) => {
  try {
    if (patch && typeof patch === 'object') {
      ...
      const allowed = ['tcpPort','detailHeight','colTs','colLvl','colLogger','windowBounds','httpUrl','httpInterval','histLogger','histTrace'];
...
ipcMain.on('tcp:start', (_event, {port}) => {
    ...
    tcpServer = net.createServer((socket) => {
        ...
        socket.on('error', (err) => {
            sendAppend([toEntry({level: 'ERROR', message: `TCP socket error: ${err.message}`}, '', 'tcp')]);
        });
    });
    ...
    tcpServer.listen(port, () => {
        tcpRunning = true; updateMenu();
        settings.tcpPort = port; saveSettings();
...
ipcMain.on('tcp:stop', (_event) => {
    ...
    tcpServer.close(() => {
        tcpServer = null; tcpRunning = false; updateMenu();
...
ipcMain.handle('http:startPoll', async (_event, {url, intervalMs}) => {
    ...
    async function tick() {
        try {
            ...
            sendAppend([toEntry({level: 'ERROR', message: `HTTP poll error for ${url}: ${err.message}`}, '', url)]);
        }
    }
